...
send_button.addEventListener("click", function() {
    if (message_input.value.trim() !== "" && num_queued_messages < 5) {
        ...
        if (user_message.trim().toLowerCase() === "about") {
        ...
        else if (user_message.trim().toLowerCase() === "reset") {
            sessionStorage.setItem('conversation_history', '');
        ...
        else if (user_message.trim().toLowerCase() === "resume") {
        ...
        else if (user_message.trim().toLowerCase() === "saaketgpt") {
            new_bot_message.textContent = "[saaket] saaketgpt is a llama 3.2 instruct model with 3 billion parameters, meaning it is a much smaller scale model than something like chatgpt. it was trained with unsloth in a google colab notebook, which is an easy method that anyone can use to train a small ai model on their own data. i first trained the llm on the finetome dataset for 1000 steps to improve the model's assistant-like qualities before doing 1000 steps on my own messages, which were scraped from discord using the browser extension discrub. overall, the model emulates my texting style reasonably for its size, which you can see for yourself by typing a message into the message bar below";
            message_area.prepend(new_bot_message);
        }
